...
class GameLogic {
	...
	/*private init(){
		...
		this.stage.addChild(propElementController);
		this.propElementController = new PropElementController(propElementController);

		let cc:egret.Sprite = new egret.Sprite();
		this.stage.addChild(cc);
		this.gameElementController = new GameElementController(cc);
		this.gameElementController.showAllElement();
		this.gameElementController.addEventListener(CustomizedEvent.TAP_TWO_ELEMENT,this.viewTouchTap,this);
	...
	private viewTouchTap(evt:CustomizedEvent){
		// 从二维地图中判断,两个元素是否可以交换位置
		let rel:boolean = EliminateLogic.isEliminated(evt.element1,evt.element2);
		if(rel){
			// 判断两个位置移动后是否可以消除
			let linerel:boolean = EliminateLogic.isEliminated(GameData.GameElements[evt.element1].location , GameData.GameElements[evt.element2].location);
			// 执行移动
			if(linerel){
				this.gameElementController.changeLocationAndScale(evt.element1,evt.element2);
				// 更新步数
				GameData.surplusStep--;
				this.levelRequirementController.updateStep();
			}
			else{
				this.gameElementController.changeLocationAndBack(evt.element1,evt.element2);
			}
		}
		else{
			// 两个元素从空洞位置上不可交换,设置新焦点
			this.gameElementController.setNewElementFocus(evt.element2);
		}
	}
