...
class GameLogic {
	...
	private mappedElementViewManagement:MappedElementViewManagement;
	private levm:LevelRequireViewManagement;
	private mapc:MapController;
	private pvm:PropViewManagement;
	...
	private init(){
		...
		GameData.init();
		// 初始化GameData数据
		let leveldata = RES.getRes("level");
		// 创建地图数据
		MapDataParse.parseMapData(leveldata.map);
		// 解析游戏关卡数据
		LevelDataParse.parseLevelData(leveldata);
		...
		this.evm.addEventListener(ElementViewManagementEvent.TAP_TWO_ELEMENT,this.viewTouchTap,this);
		this.evm.addEventListener(ElementViewManagementEvent.REMOVE_ANIMATION_OVER,this.removeAniOver,this);
		this.evm.addEventListener(ElementViewManagementEvent.UPDATE_MAP,this.createNewElement,this);
		this.evm.addEventListener(ElementViewManagementEvent.UPDATE_VIEW_OVER,this.checkOtherElementLink,this);
	...
	// 携带道具被点击
	private usePropClick(evt:ElementViewManagementEvent){
		// 操作数据
