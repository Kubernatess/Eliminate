...
var RequiredElementManagement = (function () {
    function RequiredElementManagement(container) {
        ...
        for (var i = 0; i < GameData.RequiredElements.length; i++) {
            ...
            // 将模型数据填充到视图
            element.type = GameData.RequiredElements[i].type;
            element.bitmap.texture = RES.getRes(element.type + "_png");
            element.numText.text = "×" + GameData.RequiredElements[i].num;
            // 条件元素左右两边间隔,锚点在左上角
    ...
    RequiredElementManagement.prototype.isAllElementEliminated = function () {
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (Number(this.container.getChildAt(i).numText.text) > 0) {
                return false;
            }
        }
        // 返回true表示所有元素消除了
    ...
    RequiredElementManagement.prototype.updateNumText = function (type, subtrahend) {
        ...
        for (var i = 0; i < len; i++) {
            if (this.container.getChildAt(i).type == type) {
                var num = Number(this.container.getChildAt(i).numText.text);
                this.container.getChildAt(i).numText.text = (num - subtrahend).toString();
                return;
            }
        }
    };
    ...
    RequiredElementManagement.prototype.updateElementsNum = function () {
        var len = this.container.numChildren;
        for (var i = 0; i < len; i++) {
            this.container.getChildAt(i).numText.text = "×" + GameData.RequiredElements[i].num;
        }
    };
